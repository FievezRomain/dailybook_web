...
export default async function NotFoundContent(){
    ...
    return (
            <div className="justify-self-center self-auto size-fit flex flex-col pt-32 bg-[url(/logo.png)] bg-size-[83%_auto] bg-position-[center_top_120px] bg-no-repeat">
                    ...
                    <Link href={referer} className="self-center text-xl text-white bg-baie rounded-3xl py-3 px-5 border-3 border-background size-fit mt-32">
                        Revenir sur la page {referer}
                </Link>
            </div>
            );
}