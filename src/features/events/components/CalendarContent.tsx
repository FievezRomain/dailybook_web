...
import { useEventHighlights, useEventsQuery } from "@/features/events/hooks/use-events";
...
export default function CalendarContent() {
  const { events, isLoading: isLoadingEvents, isError, error } = useEventsQuery();
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(new Date());
  const [visibleYear, setVisibleYear] = useState(new Date().getFullYear());
  const highlights = useEventHighlights(visibleYear);
  ...
  const calendarEvents = useMemo(() =>
    [
      ...
      ...(highlights.data ?? []).map((highlight) => ({
        ...
        extendedProps: { highlight: true, kind: highlight.kind },
      })),
    ],
    [events, highlights.data]
  );
  ...
  if (isError) {
    return <div role="alert" className="m-7 rounded-xl bg-destructive/10 p-6 text-destructive">
      {error instanceof Error ? error.message : "Impossible de charger les événements."}
    </div>;
  }
      ...
      <div className="md:w-2/3 w-full flex flex-col h-full">
        <div className="flex-1 w-full h-full bg-card rounded-2xl p-6 shadow-sm dark:shadow-lg dark:shadow-black/30">
          <FullCalendar
            ...
            datesSet={(info) => setVisibleYear(info.view.currentStart.getFullYear())}
